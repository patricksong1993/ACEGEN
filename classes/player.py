...
class Player:
    ...
    def __init__(self, name):
        ...
        self.game_impact = {}
        self.point_impact = {}
        self.tiebreak_impact = 0

        # two-step baselines
        self.match_baseline_ts = 0
        self.set_baseline_ts = {}
        self.set_baseline_3_ts = {}
        self.game_baseline_ts = {}
        self.point_baseline_ts = {}
        self.tiebreak_baseline_ts = 0

        # two-step impacts
        self.match_impact_ts = 0
        self.set_impact_ts = {}
        self.set_impact_3_ts = {}
        self.game_impact_ts = {}
        self.point_impact_ts = {}
        self.tiebreak_impact_ts = 0

        self.enrich_matches()
        self.calculate_baseline()
        self.calculate_impact()
        # self.calculate_baseline_ts()
    ...
    def enrich_matches(self):
        ...
        if len(self.match_ids[1]) == 0 and len(self.match_ids[2]) == 0 and len(self.match_ids[3]) == 0 and len(self.match_ids[4]) == 0:
    ...
    def calculate_baseline_ts(self):
        match_baseline_dp = match_baseline_ts(self.match_ids)
        self.match_baseline_ts = PlayerStats(match_baseline_dp[0], match_baseline_dp[1])

        [set_baseline_dp, set_baseline_3_dp] = set_baseline_ts(self.match_ids)
        for set in set_baseline_dp:
            self.set_baseline_ts[set] = PlayerStats(set_baseline_dp[set][0], set_baseline_dp[set][1])

        for set in set_baseline_3_dp:
            self.set_baseline_3_ts[set] = PlayerStats(set_baseline_3_dp[set][0], set_baseline_3_dp[set][1])

        game_baseline_dp = game_baseline_ts(self.match_ids)
        for game in game_baseline_dp:
            self.game_baseline_ts[game] = PlayerStats2(game_baseline_dp[game][0], game_baseline_dp[game][1])

        # hacking
        scenarios = [3, 2, 1, 0, -1, -2, -3]
        for s in scenarios:
            if self.game_baseline_ts.get(s, 0) == 0:
                self.game_baseline_ts[s] = PlayerStats2([], [])

        point_baseline_dp = point_baseline_ts(self.match_ids)
        for point in point_baseline_dp:
            self.point_baseline_ts[point] = PlayerStats2(point_baseline_dp[point][0], point_baseline_dp[point][1])

    def calculate_impact_ts(self):
        match_impact_dp = match_impact_ts(self.match_ids)
        self.match_impact_ts = PlayerStats(match_impact_dp[0], match_impact_dp[1])

        [set_impact_dp, set_impact_3_dp] = set_impact_ts(self.match_ids)
        for set in set_impact_dp:
            self.set_impact_ts[set] = PlayerStats(set_impact_dp[set][0], set_impact_dp[set][1])

        for set in set_impact_3_dp:
            self.set_impact_3_ts[set] = PlayerStats(set_impact_3_dp[set][0], set_impact_3_dp[set][1])

        game_impact_dp = game_impact_ts(self.match_ids)
        for game in game_impact_dp:
            self.game_impact_ts[game] = PlayerStats2(game_impact_dp[game][0], game_impact_dp[game][1])
        ...
        point_impact_dp = point_impact_ts(self.match_ids)
        for point in point_impact_dp:
            self.point_impact_ts[point] = PlayerStats2(point_impact_dp[point][0], point_impact_dp[point][1])
    ...
    @staticmethod
    def calculate_all_ts():
        for player in Player.cached_players:
            if Player.cached_players[player].match_baseline_ts == 0:
                Player.cached_players[player].calculate_baseline_ts()
        for player in Player.cached_players:
            if Player.cached_players[player].match_impact_ts == 0:
                Player.cached_players[player].calculate_impact_ts()
...
def match_baseline_ts(match_ids):
    serv = []
    retn = []
    matches_1 = Match.get_matches(match_ids[1])
    for match in matches_1:
        serv.append(matches_1[match].match_pct.p1_serv_pct - Player.get_player(matches_1[match].player2).match_impact.retn_mean)
        retn.append(matches_1[match].match_pct.p1_retn_pct - Player.get_player(matches_1[match].player2).match_impact.serv_mean)
    matches_2 = Match.get_matches(match_ids[2])
    for match in matches_2:
        serv.append(matches_2[match].match_pct.p2_serv_pct - Player.get_player(matches_2[match].player1).match_impact.retn_mean)
        retn.append(matches_2[match].match_pct.p2_retn_pct - Player.get_player(matches_2[match].player1).match_impact.serv_mean)
    matches_3 = Match.get_matches_pbp(match_ids[3])
    for match in matches_3:
        serv.append(matches_3[match].match_pct.p1_serv_pct - Player.get_player(matches_3[match].player2).match_impact.retn_mean)
        retn.append(matches_3[match].match_pct.p1_retn_pct - Player.get_player(matches_3[match].player2).match_impact.serv_mean)
    matches_4 = Match.get_matches_pbp(match_ids[4])
    for match in matches_4:
        serv.append(matches_4[match].match_pct.p2_serv_pct - Player.get_player(matches_4[match].player1).match_impact.retn_mean)
        retn.append(matches_4[match].match_pct.p2_retn_pct - Player.get_player(matches_4[match].player1).match_impact.serv_mean)
    return [serv, retn]
...
def set_baseline_ts(match_ids):
    ...
    for match in matches_1:
        for set in matches_1[match].set_pct:
            cur = set_baseline_dp.get(set, [[], []])
            cur[0].append(matches_1[match].set_pct[set].p1_serv_pct - Player.get_player(matches_1[match].player2).set_impact[inv_scr(set)].retn_mean)
            cur[1].append(matches_1[match].set_pct[set].p1_retn_pct - Player.get_player(matches_1[match].player2).set_impact[inv_scr(set)].serv_mean)
            set_baseline_dp[set] = cur
    matches_2 = Match.get_matches(match_ids[2])
    for match in matches_2:
        for set in matches_2[match].set_pct:
            # inversing score as player 2
            cur = set_baseline_dp.get(inv_scr(set), [[], []])
            cur[0].append(matches_2[match].set_pct[set].p2_serv_pct - Player.get_player(matches_2[match].player1).set_impact[set].retn_mean)
            cur[1].append(matches_2[match].set_pct[set].p2_retn_pct - Player.get_player(matches_2[match].player1).set_impact[set].serv_mean)
            set_baseline_dp[inv_scr(set)] = cur
    matches_3 = Match.get_matches(match_ids[3])
    for match in matches_3:
        for set in matches_3[match].set_pct:
            cur = set_baseline_3_dp.get(set, [[], []])
            cur[0].append(matches_3[match].set_pct[set].p1_serv_pct - Player.get_player(matches_3[match].player2).set_impact_3[inv_scr(set)].retn_mean)
            cur[1].append(matches_3[match].set_pct[set].p1_retn_pct - Player.get_player(matches_3[match].player2).set_impact_3[inv_scr(set)].serv_mean)
            set_baseline_3_dp[set] = cur
    matches_4 = Match.get_matches(match_ids[4])
    for match in matches_4:
        for set in matches_4[match].set_pct:
            # inversing score as player 2
            cur = set_baseline_3_dp.get(inv_scr(set), [[], []])
            cur[0].append(matches_4[match].set_pct[set].p2_serv_pct - Player.get_player(matches_4[match].player1).set_impact_3[set].retn_mean)
            cur[1].append(matches_4[match].set_pct[set].p2_retn_pct - Player.get_player(matches_4[match].player1).set_impact_3[set].serv_mean)
            set_baseline_3_dp[inv_scr(set)] = cur
    return [set_baseline_dp, set_baseline_3_dp]
...
def game_baseline_ts(match_ids):
    game_baseline_dp = {}
    matches_1 = Match.get_matches(match_ids[1])
    for match in matches_1:
        for game in matches_1[match].game_pct:
            cur = game_baseline_dp.get(game, [[], []])
            serv_dp = [1] * matches_1[match].game_pct[game].p1_serv_w + [0] * (matches_1[match].game_pct[game].p1_serv - matches_1[match].game_pct[game].p1_serv_w)
            serv_dp = [x - Player.get_player(matches_1[match].player2).game_impact[inv_scr(game)].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_1[match].game_pct[game].p1_retn_w + [0] * (matches_1[match].game_pct[game].p1_retn - matches_1[match].game_pct[game].p1_retn_w)
            retn_dp = [x - Player.get_player(matches_1[match].player2).game_impact[inv_scr(game)].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            game_baseline_dp[game] = cur
    matches_2 = Match.get_matches(match_ids[2])
    for match in matches_2:
        for game in matches_2[match].game_pct:
            # inversing score as player 2
            cur = game_baseline_dp.get(inv_scr(game), [[], []])
            serv_dp = [1] * matches_2[match].game_pct[game].p2_serv_w + [0] * (matches_2[match].game_pct[game].p2_serv - matches_2[match].game_pct[game].p2_serv_w)
            serv_dp = [x - Player.get_player(matches_2[match].player1).game_impact[game].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_2[match].game_pct[game].p2_retn_w + [0] * (matches_2[match].game_pct[game].p2_retn - matches_2[match].game_pct[game].p2_retn_w)
            retn_dp = [x - Player.get_player(matches_2[match].player1).game_impact[game].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            game_baseline_dp[inv_scr(game)] = cur
    matches_3 = Match.get_matches(match_ids[3])
    for match in matches_3:
        for game in matches_3[match].game_pct:
            cur = game_baseline_dp.get(game, [[], []])
            serv_dp = [1] * matches_3[match].game_pct[game].p1_serv_w + [0] * (matches_3[match].game_pct[game].p1_serv - matches_3[match].game_pct[game].p1_serv_w)
            serv_dp = [x - Player.get_player(matches_3[match].player2).game_impact[inv_scr(game)].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_3[match].game_pct[game].p1_retn_w + [0] * (matches_3[match].game_pct[game].p1_retn - matches_3[match].game_pct[game].p1_retn_w)
            retn_dp = [x - Player.get_player(matches_3[match].player2).game_impact[inv_scr(game)].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            game_baseline_dp[game] = cur
    matches_4 = Match.get_matches(match_ids[4])
    for match in matches_4:
        for game in matches_4[match].game_pct:
            # inversing score as player 2
            cur = game_baseline_dp.get(inv_scr(game), [[], []])
            serv_dp = [1] * matches_4[match].game_pct[game].p2_serv_w + [0] * (matches_4[match].game_pct[game].p2_serv - matches_4[match].game_pct[game].p2_serv_w)
            serv_dp = [x - Player.get_player(matches_4[match].player1).game_impact[game].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_4[match].game_pct[game].p2_retn_w + [0] * (matches_4[match].game_pct[game].p2_retn - matches_4[match].game_pct[game].p2_retn_w)
            retn_dp = [x - Player.get_player(matches_4[match].player1).game_impact[game].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            game_baseline_dp[inv_scr(game)] = cur
    return game_baseline_dp
...
def point_baseline_ts(match_ids):
    point_baseline_dp = {}
    matches_1 = Match.get_matches(match_ids[1])
    for match in matches_1:
        for point in matches_1[match].point_pct:
            cur = point_baseline_dp.get(point, [[], []])
            serv_dp = [1] * matches_1[match].point_pct[point].p1_serv_w + [0] * (matches_1[match].point_pct[point].p1_serv - matches_1[match].point_pct[point].p1_serv_w)
            serv_dp = [x - Player.get_player(matches_1[match].player2).point_impact[inv_scr(point)].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_1[match].point_pct[point].p1_retn_w + [0] * (matches_1[match].point_pct[point].p1_retn - matches_1[match].point_pct[point].p1_retn_w)
            retn_dp = [x - Player.get_player(matches_1[match].player2).point_impact[inv_scr(point)].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            point_baseline_dp[point] = cur
    matches_2 = Match.get_matches(match_ids[2])
    for match in matches_2:
        for point in matches_2[match].point_pct:
            # inversing score as player 2
            cur = point_baseline_dp.get(inv_scr(point), [[], []])
            serv_dp = [1] * matches_2[match].point_pct[point].p2_serv_w + [0] * (matches_2[match].point_pct[point].p2_serv - matches_2[match].point_pct[point].p2_serv_w)
            serv_dp = [x - Player.get_player(matches_2[match].player1).point_impact[point].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_2[match].point_pct[point].p2_retn_w + [0] * (matches_2[match].point_pct[point].p2_retn - matches_2[match].point_pct[point].p2_retn_w)
            retn_dp = [x - Player.get_player(matches_2[match].player1).point_impact[point].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            point_baseline_dp[inv_scr(point)] = cur
    matches_3 = Match.get_matches(match_ids[3])
    for match in matches_3:
        for point in matches_3[match].point_pct:
            cur = point_baseline_dp.get(point, [[], []])
            serv_dp = [1] * matches_3[match].point_pct[point].p1_serv_w + [0] * (matches_3[match].point_pct[point].p1_serv - matches_3[match].point_pct[point].p1_serv_w)
            serv_dp = [x - Player.get_player(matches_3[match].player2).point_impact[inv_scr(point)].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_3[match].point_pct[point].p1_retn_w + [0] * (matches_3[match].point_pct[point].p1_retn - matches_3[match].point_pct[point].p1_retn_w)
            retn_dp = [x - Player.get_player(matches_3[match].player2).point_impact[inv_scr(point)].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            point_baseline_dp[point] = cur
    matches_4 = Match.get_matches(match_ids[4])
    for match in matches_4:
        for point in matches_4[match].point_pct:
            # inversing score as player 2
            cur = point_baseline_dp.get(inv_scr(point), [[], []])
            serv_dp = [1] * matches_4[match].point_pct[point].p2_serv_w + [0] * (matches_4[match].point_pct[point].p2_serv - matches_4[match].point_pct[point].p2_serv_w)
            serv_dp = [x - Player.get_player(matches_4[match].player1).point_impact[point].retn_mean for x in serv_dp]
            retn_dp = [1] * matches_4[match].point_pct[point].p2_retn_w + [0] * (matches_4[match].point_pct[point].p2_retn - matches_4[match].point_pct[point].p2_retn_w)
            retn_dp = [x - Player.get_player(matches_4[match].player1).point_impact[point].serv_mean for x in retn_dp]
            cur[0].extend(serv_dp)
            cur[1].extend(retn_dp)
            point_baseline_dp[inv_scr(point)] = cur
    return point_baseline_dp
...
def match_impact_ts(match_ids):
    ...
    for match in matches_1:
        oppo = Player.get_player(matches_1[match].player2)
        serv.append(1.0 - matches_1[match].match_pct.p1_retn_pct - oppo.match_baseline_ts.serv_mean)
        retn.append(1.0 - matches_1[match].match_pct.p1_serv_pct - oppo.match_baseline_ts.retn_mean)

    matches_2 = Match.get_matches(match_ids[2])
    for match in matches_2:
        oppo = Player.get_player(matches_2[match].player1)
        serv.append(1.0 - matches_2[match].match_pct.p2_retn_pct - oppo.match_baseline_ts.serv_mean)
        retn.append(1.0 - matches_2[match].match_pct.p2_serv_pct - oppo.match_baseline_ts.retn_mean)

    matches_3 = Match.get_matches(match_ids[3])
    for match in matches_3:
        oppo = Player.get_player(matches_3[match].player2)
        serv.append(1.0 - matches_3[match].match_pct.p1_retn_pct - oppo.match_baseline_ts.serv_mean)
        retn.append(1.0 - matches_3[match].match_pct.p1_serv_pct - oppo.match_baseline_ts.retn_mean)

    matches_4 = Match.get_matches(match_ids[4])
    for match in matches_4:
        oppo = Player.get_player(matches_4[match].player1)
        serv.append(1.0 - matches_4[match].match_pct.p2_retn_pct - oppo.match_baseline_ts.serv_mean)
        retn.append(1.0 - matches_4[match].match_pct.p2_serv_pct - oppo.match_baseline_ts.retn_mean)
    return [serv, retn]
...
def set_impact_ts(match_ids):
    ...
    for match in matches_1:
        oppo = Player.get_player(matches_1[match].player2)
        for set in matches_1[match].set_pct:
            serv = 1.0 - matches_1[match].set_pct[set].p1_retn_pct - oppo.set_baseline_ts[inv_scr(set)].serv_mean
            retn = 1.0 - matches_1[match].set_pct[set].p1_serv_pct - oppo.set_baseline_ts[inv_scr(set)].retn_mean
            cur = impact.get(set, [[], []])
            cur[0].append(serv)
            cur[1].append(retn)
            impact[set] = cur
    ...
    for match in matches_2:
        oppo = Player.get_player(matches_2[match].player1)
        for set in matches_2[match].set_pct:
            serv = 1.0 - matches_2[match].set_pct[set].p2_retn_pct - oppo.set_baseline_ts[set].serv_mean
            retn = 1.0 - matches_2[match].set_pct[set].p2_serv_pct - oppo.set_baseline_ts[set].retn_mean
            cur = impact.get(inv_scr(set), [[], []])
            cur[0].append(serv)
            cur[1].append(retn)
            impact[inv_scr(set)] = cur
    ...
    for match in matches_3:
        oppo = Player.get_player(matches_3[match].player2)
        for set in matches_3[match].set_pct:
            serv = 1.0 - matches_3[match].set_pct[set].p1_retn_pct - oppo.set_baseline_3_ts[inv_scr(set)].serv_mean
            retn = 1.0 - matches_3[match].set_pct[set].p1_serv_pct - oppo.set_baseline_3_ts[inv_scr(set)].retn_mean
            cur = impact_3.get(set, [[], []])
            cur[0].append(serv)
            cur[1].append(retn)
            impact_3[set] = cur
    ...
    for match in matches_4:
        oppo = Player.get_player(matches_4[match].player1)
        for set in matches_4[match].set_pct:
            serv = 1.0 - matches_4[match].set_pct[set].p2_retn_pct - oppo.set_baseline_3_ts[set].serv_mean
            retn = 1.0 - matches_4[match].set_pct[set].p2_serv_pct - oppo.set_baseline_3_ts[set].retn_mean
            cur = impact_3.get(inv_scr(set), [[], []])
            cur[0].append(serv)
            cur[1].append(retn)
            impact_3[inv_scr(set)] = cur
...
def game_impact_ts(match_ids):
    ...
    for match in matches_1:
        oppo = Player.get_player(matches_1[match].player2)
        for game in matches_1[match].game_pct:
            serv = [0.0 - oppo.game_baseline_ts[inv_scr(game)].serv_mean] * matches_1[match].game_pct[game].p1_retn_w + \
                   [1.0 - oppo.game_baseline_ts[inv_scr(game)].serv_mean] * (matches_1[match].game_pct[game].p1_retn - matches_1[match].game_pct[game].p1_retn_w)
            retn = [0.0 - oppo.game_baseline_ts[inv_scr(game)].retn_mean] * matches_1[match].game_pct[game].p1_serv_w + \
                   [1.0 - oppo.game_baseline_ts[inv_scr(game)].retn_mean] * (matches_1[match].game_pct[game].p1_serv - matches_1[match].game_pct[game].p1_serv_w)
            cur = impact.get(game, [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[game] = cur
    ...
    for match in matches_2:
        oppo = Player.get_player(matches_2[match].player1)
        for game in matches_2[match].game_pct:
            serv = [0.0 - oppo.game_baseline_ts[game].serv_mean] * matches_2[match].game_pct[game].p2_retn_w + \
                   [1.0 - oppo.game_baseline_ts[game].serv_mean] * (matches_2[match].game_pct[game].p2_retn - matches_2[match].game_pct[game].p2_retn_w)
            retn = [0.0 - oppo.game_baseline_ts[game].retn_mean] * matches_2[match].game_pct[game].p2_serv_w + \
                   [1.0 - oppo.game_baseline_ts[game].retn_mean] * (matches_2[match].game_pct[game].p2_serv - matches_2[match].game_pct[game].p2_serv_w)
            cur = impact.get(inv_scr(game), [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[inv_scr(game)] = cur
    ...
    for match in matches_3:
        oppo = Player.get_player(matches_3[match].player2)
        for game in matches_3[match].game_pct:
            serv = [0.0 - oppo.game_baseline_ts[inv_scr(game)].serv_mean] * matches_3[match].game_pct[game].p1_retn_w + \
                   [1.0 - oppo.game_baseline_ts[inv_scr(game)].serv_mean] * (matches_3[match].game_pct[game].p1_retn - matches_3[match].game_pct[game].p1_retn_w)
            retn = [0.0 - oppo.game_baseline_ts[inv_scr(game)].retn_mean] * matches_3[match].game_pct[game].p1_serv_w + \
                   [1.0 - oppo.game_baseline_ts[inv_scr(game)].retn_mean] * (matches_3[match].game_pct[game].p1_serv - matches_3[match].game_pct[game].p1_serv_w)
            cur = impact.get(game, [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[game] = cur
    ...
    for match in matches_4:
        oppo = Player.get_player(matches_4[match].player1)
        for game in matches_4[match].game_pct:
            serv = [0.0 - oppo.game_baseline_ts[game].serv_mean] * matches_4[match].game_pct[game].p2_retn_w + \
                   [1.0 - oppo.game_baseline_ts[game].serv_mean] * (matches_4[match].game_pct[game].p2_retn - matches_4[match].game_pct[game].p2_retn_w)
            retn = [0.0 - oppo.game_baseline_ts[game].retn_mean] * matches_4[match].game_pct[game].p2_serv_w + \
                   [1.0 - oppo.game_baseline_ts[game].retn_mean] * (matches_4[match].game_pct[game].p2_serv - matches_4[match].game_pct[game].p2_serv_w)
            cur = impact.get(inv_scr(game), [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[inv_scr(game)] = cur
...
def point_impact_ts(match_ids):
    ...
    for match in matches_1:
        oppo = Player.get_player(matches_1[match].player2)
        for point in matches_1[match].point_pct:
            serv = [0.0 - oppo.point_baseline_ts[inv_scr(point)].serv_mean] * matches_1[match].point_pct[point].p1_retn_w + \
                   [1.0 - oppo.point_baseline_ts[inv_scr(point)].serv_mean] * (matches_1[match].point_pct[point].p1_retn - matches_1[match].point_pct[point].p1_retn_w)
            retn = [0.0 - oppo.point_baseline_ts[inv_scr(point)].retn_mean] * matches_1[match].point_pct[point].p1_serv_w + \
                   [1.0 - oppo.point_baseline_ts[inv_scr(point)].retn_mean] * (matches_1[match].point_pct[point].p1_serv - matches_1[match].point_pct[point].p1_serv_w)
            cur = impact.get(point, [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[point] = cur
    ...
    for match in matches_2:
        oppo = Player.get_player(matches_2[match].player1)
        for point in matches_2[match].point_pct:
            serv = [0.0 - oppo.point_baseline_ts[point].serv_mean] * matches_2[match].point_pct[point].p2_retn_w + \
                   [1.0 - oppo.point_baseline_ts[point].serv_mean] * (matches_2[match].point_pct[point].p2_retn - matches_2[match].point_pct[point].p2_retn_w)
            retn = [0.0 - oppo.point_baseline_ts[point].retn_mean] * matches_2[match].point_pct[point].p2_serv_w + \
                   [1.0 - oppo.point_baseline_ts[point].retn_mean] * (matches_2[match].point_pct[point].p2_serv - matches_2[match].point_pct[point].p2_serv_w)
            cur = impact.get(inv_scr(point), [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[inv_scr(point)] = cur
    ...
    for match in matches_3:
        oppo = Player.get_player(matches_3[match].player2)
        for point in matches_3[match].point_pct:
            serv = [0.0 - oppo.point_baseline_ts[inv_scr(point)].serv_mean] * matches_3[match].point_pct[point].p1_retn_w + \
                   [1.0 - oppo.point_baseline_ts[inv_scr(point)].serv_mean] * (matches_3[match].point_pct[point].p1_retn - matches_3[match].point_pct[point].p1_retn_w)
            retn = [0.0 - oppo.point_baseline_ts[inv_scr(point)].retn_mean] * matches_3[match].point_pct[point].p1_serv_w + \
                   [1.0 - oppo.point_baseline_ts[inv_scr(point)].retn_mean] * (matches_3[match].point_pct[point].p1_serv - matches_3[match].point_pct[point].p1_serv_w)
            cur = impact.get(point, [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[point] = cur
    ...
    for match in matches_4:
        oppo = Player.get_player(matches_4[match].player1)
        for point in matches_4[match].point_pct:
            serv = [0.0 - oppo.point_baseline_ts[point].serv_mean] * matches_4[match].point_pct[point].p2_retn_w + \
                   [1.0 - oppo.point_baseline_ts[point].serv_mean] * (matches_4[match].point_pct[point].p2_retn - matches_4[match].point_pct[point].p2_retn_w)
            retn = [0.0 - oppo.point_baseline_ts[point].retn_mean] * matches_4[match].point_pct[point].p2_serv_w + \
                   [1.0 - oppo.point_baseline_ts[point].retn_mean] * (matches_4[match].point_pct[point].p2_serv - matches_4[match].point_pct[point].p2_serv_w)
            cur = impact.get(inv_scr(point), [[], []])
            cur[0].extend(serv)
            cur[1].extend(retn)
            impact[inv_scr(point)] = cur
